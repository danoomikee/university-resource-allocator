...
export const personnelStorage = {
  ...
  getById: (id: string): Personnel | undefined => {
    return getFromStorage<Personnel>(STORAGE_KEYS.PERSONNEL).find(
      (p) => p.id === id
    );
  },
...
export const userStorage = {
  getAll: (universityId?: string): UserAccount[] => {
    ...
    // return universityId
    // ? users.filter((u) => u.universityId === universityId)
    // : users;
    return users;
  },

  getById: (id: string): UserAccount | undefined => {
    return getFromStorage<UserAccount>(STORAGE_KEYS.USERS).find(
      (u) => u.id === id
    );
  },
...
export const entityAssignmentStorage = {
  ...
  create: (
    assignment: Omit<EntityAssignment, "id" | "createdAt" | "updatedAt">
  ): EntityAssignment => {
    const assignments = getFromStorage<EntityAssignment>(
      STORAGE_KEYS.ENTITY_ASSIGNMENTS
    );
  ...
  update: (
    id: string,
    updates: Partial<EntityAssignment>
...
export const courseStorage = {
  ...
  getById: (id: string): Course | undefined => {
    return getFromStorage<Course>(STORAGE_KEYS.COURSES).find(
      (c) => c.id === id
    );
  },
...
export const offeringStorage = {
  ...
  getById: (id: string): Offering | undefined => {
    return getFromStorage<Offering>(STORAGE_KEYS.OFFERINGS).find(
      (o) => o.id === id
    );
  },

  create: (
    offering: Omit<Offering, "id" | "createdAt" | "updatedAt">
  ): Offering => {
    const offerings = getFromStorage<Offering>(STORAGE_KEYS.OFFERINGS);
