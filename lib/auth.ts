...
export const login = (
  ...
  if (entityId) {
    const assignment = entityAssignmentStorage.getByUserAndEntity(
      user.id,
      entityId,
      user.universityId
    );
  ...
  return {
    success: false,
    error: "Entity assignment required for EntityAdmin login",
  };
};
...
export const getCurrentUserAssignment = (): EntityAssignment | null => {
  ...
  const assignments = entityAssignmentStorage.getByUserId(
    user.id,
    user.universityId
  );
...
export const belongsToUniversity = (
  userId: string,
  universityId: string
): boolean => {
...
export const getUniversityContext = (): {
  user: UserAccount;
...
export const getUserAssignedEntities = (
  userId: string,
  universityId: string
): Entity[] => {
  const assignments = entityAssignmentStorage.getByUserId(userId, universityId);
  return assignments
    .map((a) => entityStorage.getById(a.entityId))
...
export const getEntityManagers = (
  ...
  const assignments = entityAssignmentStorage.getByEntityId(
    entityId,
    universityId
  );
  return assignments
    ...
    .filter(
      (item): item is UserAccount & { assignment: EntityAssignment } =>
        item !== null
    );
};
